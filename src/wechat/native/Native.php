...
class Native implements WeChatComm, PaymentComm
{
    ...
    public function purchase($data)
    {
        ...
        if ($order["trade_type"] == "JSAPI") {
            if (!array_key_exists("openid", $order)) {
                throw new \InvalidArgumentException("When you use JSAPI, the openid field is required");
            }
        }

        if ($order["trade_type"] == "NATIVE") {
            if (!array_key_exists("product_id", $order)) {
                throw new \InvalidArgumentException("When you use NATIVE, the product_id field is required");
            }
        }
    ...
    {
        return $this->sendRequest($this->refundquery_url, "POST", $order, "", $this->api_key);
    }
